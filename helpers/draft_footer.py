...
Drafts are identified by their 8-character Draftmancer code rather than the full
`session_id` (`{user_id}-{unix_ts}`), which is long and embeds the initiating
user's Discord ID.
"""
...
def draft_footer_text(draft_id, cube):
    ...
    parts = []
    if draft_id:
        parts.append(f"ID: {draft_id}")
    if cube:
        parts.append(f"Cube: {cube}")
    return SEPARATOR.join(parts)


def apply_draft_footer(embed, draft_id, cube):
    """Stamp a draft's metadata onto an embed's footer."""
    text = draft_footer_text(draft_id, cube)
    if text:
...
def apply_draft_footer_from_session(embed, draft_session):
    ...
    return apply_draft_footer(embed, draft_session.draft_id, draft_session.cube)
