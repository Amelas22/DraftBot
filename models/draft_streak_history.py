...
class DraftStreakHistory(Base):
    __tablename__ = 'draft_streak_history'
    ...
    __table_args__ = (
        Index('idx_draft_streak_player_guild', 'player_id', 'guild_id'),
        Index('idx_draft_streak_guild_started', 'guild_id', 'started_at'),
        Index('idx_draft_streak_guild_ended', 'guild_id', 'ended_at'),
    )

    def __repr__(self):
        status = "active" if self.ended_at is None else "ended"
        return f"<DraftStreakHistory({self.player_id}: {self.streak_length} draft wins, {status})>"
