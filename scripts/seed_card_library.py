...
It is deliberately not a way to run the library. The donor it books the cards
against is fictional, so a deck seeded here has nobody to give them back to,
and the vault is not decremented. Use it against the test guild.

It DOES book those cards to the library's ledger, because it has to: what a
library can lend is the lesser of what the account physically holds and what
the ledger says this library is owed. Seeding only the loan leaves the library
entitled to nothing and every /borrow refuses for want of stock -- a setup
failure that reads exactly like a broken feature.
...
from services.library_service import library_for  # noqa: E402
...
async def main():
    ...
    library = await library_for(args.guild)
    if library is None:
        print(f"REFUSED: {args.guild} is not bound to a library. "
              f"Run scripts/library_tool.py bind first.")
        return 1

    for user, deck, _ in plan:
        # Book the library as OWED these cards before assigning them.
        #
        # What a library can lend is the lesser of what the MTGO account
        # physically holds and what the LEDGER says this library is owed -- one
        # account holds every library's stock commingled, so the vault alone
        # cannot say whose a card is. A fixture that writes only the loan
        # leaves the library entitled to nothing, and every seeded /borrow
        # refuses with "short_cards": a setup failure that reads exactly like
        # a broken feature.
        #
        # The donor is fictional, which is the honest shape for a fixture --
        # there is nobody to give these back to. A real shelf is built by
        # /deposit, which records the player who handed them over.
        for card in deck:
            await debt_service.create_card_loan(
                guild_id=wallet_service.library_scope(library.id),
                lender_id=f"fixture:donor:{args.guild}",
                borrower_id=wallet_service.HOUSE_LIBRARY,
                card_name=card["name"], quantity=int(card["qty"]),
                created_by="seed_card_library", 
                source_id=f"fixture:{library.id}:{user}:{card['name']}")
        loan_id = await assign_deck(args.guild, user, deck, source=f"fixture:{user}")
        print(f"  assigned loan {loan_id} to {user} "
              f"(and booked {len(deck)} card row(s) to {library.id})")
