...
async def _holding_a_deck(guild_id: Any, borrower_ids: "list[str]") -> "set[str]":
    """Which of these already have an unfinished loan.
...
async def assign_drafted_decks(session_id: Any) -> int:
    ...
    busy = await _holding_a_deck(guild_id, candidates)
    ...
    for discord_id in assignable:
        ...
        if too_large(total):
            ...
            continue
        try:
            await assign_deck(guild_id, discord_id, cards, source=_source(session_id))
            ...
            # at every await. The loser hits the one-active-loan index. Benign:
            # the row it collided with is the deck being assigned right now.
            logger.info("deck assignment: {} was assigned {} concurrently",
