...
def run_stake_simulations(players, min_stake=10, multiple=10):
    ...
    for i in range(NUM_SIMULATIONS):
        ...
        log_capture.stop()
        simulation_log = log_capture.get_logs()
        ...
        sim_result = {
            ...
            'stake_pairs': [f"{p.player_a_id} vs {p.player_b_id}: {p.amount} tix" for p in stake_pairs],
            'total_stake': sum(p.amount for p in stake_pairs),
            'num_pairs': len(stake_pairs),
...
def write_to_excel(players, results, logs):
    ...
    for i, (result, simulation_log) in enumerate(zip(results, logs)):
        ...
        # Add stake pairs
        sim_sheet.append(["BottomsUp Stake Pairs"])
        ...
        # Add log sheet
        log_sheet = wb.create_sheet(f"Sim {i+1} Log")
        
        # Add logs
        log_sheet.append(["BottomsUp Algorithm Logs"])
        log_lines = simulation_log.strip().split('\n')
    ...
    filename = f"bottoms_up_simulations_{timestamp}.xlsx"
...
def main():
    print("Welcome to the BottomsUp Stake Algorithm Tester")
    print("==============================================")
