...
async def test_the_library_does_not_claim_the_house_features_jobs(test_db):
    ...
    async with AsyncSessionLocal() as s:
        loan = CardLoan(guild_id=GUILD, library_id="lib", borrower_id=BORROWER,
                        cards=[{"name": "Swamp", "qty": 8}], state="out_pending",
                        source="fixture")
        s.add(loan)
        ...
        # No order stamp, so it is matched by borrower -- which is exactly the
        # case this is about: the card_name is what tells the two apart.
    ...
    async with AsyncSessionLocal() as s:
        found = [j.job_id for j in await svc._batches_for(await s.get(CardLoan, loan_id))]
