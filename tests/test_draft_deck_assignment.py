...
GUILD, SESSION, LIBRARY = "g1", "sess-1", "lib"
...
async def library_on(test_db):
    """Bind this guild to a library that offers the cube.

    There is no guild-wide switch any more, and no per-cube price either: a
    server draws on one LIBRARY and is bound to its terms, set by whoever runs
    it from a shell tool -- because a server admin can rewrite their own config
    through the bot, and anything they can reach is a number they can lower.
    """
    from conftest import a_library
    await a_library(LIBRARY, guild=GUILD, collateral=5, cubes=(CUBE,))
    yield
...
async def test_a_server_that_has_not_been_priced_for_this_cube_is_left_alone(
        test_db):
    """Opting in is per CUBE, not per server, and there is no guild-wide switch
    to flip. A server nobody has priced this cube for gets nothing -- which is
    also what stops a cheap cube being used as a route into the expensive
    shelf, since each cube is opted in on its own terms."""
    await _seed()
...
async def test_a_draft_with_no_cube_recorded_is_left_alone(test_db, library_on):
    """Nothing to price it by, so nothing to charge, so no deck."""
    await _seed()
...
async def test_a_deck_held_in_ANOTHER_server_counts_as_holding_one(test_db,
                                                                   library_on):
    """The one-loan-at-a-time rule is global, and this skip has to match it.

    card_loans' unique index is on borrower_id alone -- the library is a single
    MTGO account, so a deck out in server A is out, full stop. Filtering the
    skip by guild did not hand anyone the wrong deck (the index still refused
    the row) but it sent them down the collision path, where the cause is
    logged as a concurrent assignment -- a different, benign thing -- and the
    drafter is counted in neither the skips nor the failures. Nobody looking at
    the tally afterwards could tell they had been passed over, or why.
    """
    # The other server draws on the same library -- which is the point: one
    # MTGO account, so a deck out anywhere is out. An UNBOUND server cannot
    # assign at all, which is a different rule with its own test.
    from models.library_server import LibraryServer
    async with AsyncSessionLocal() as s:
        s.add(LibraryServer(guild_id="other-guild", library_id=LIBRARY,
                            bound_by="test"))
        await s.commit()
    await assign_deck("other-guild", ALICE, [{"name": "Swamp", "qty": 4}],
                      source="earlier")

    assert await svc._holding_a_deck([ALICE, BOB]) == {ALICE}
...
async def test_one_players_write_failing_still_assigns_everybody_else(
    ...
    async def flaky(guild_id, borrower_id, cards, source=None, library_id=None):
        calls["n"] += 1
        if calls["n"] == 1:
