...
# A loan always knows its library: settling refuses a trade whose shelf it
# cannot confirm, so a fixture without one is a corrupt row, not a lean one.
LIB = "lib"
JOB = "gone-job"
...
async def _seed(state, job_id=JOB):
    async with AsyncSessionLocal() as s:
        loan = CardLoan(guild_id=GUILD, borrower_id=BORROWER, cards=DECK,
                        library_id=LIB, state=state, job_id=job_id,
                        source="fixture")
        s.add(loan)
        await s.commit()
        return loan.id
