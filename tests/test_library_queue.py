...
async def test_a_free_library_dispatches_straight_away(monkeypatch):
    ...
    monkeypatch.setattr(svc, "_dispatch", started)
...
async def test_two_borrowers_do_not_race_for_the_same_serve(monkeypatch):
    ...
    async def slow_start(guild_id, borrower_id, **kw):
        inside["now"] += 1
    ...
    monkeypatch.setattr(svc, "library_busy_reason", AsyncMock(return_value=None))
    monkeypatch.setattr(svc, "_dispatch", slow_start)
...
async def test_a_return_does_not_race_a_borrow_for_the_same_serve(monkeypatch):
    ...
    async def slow(guild_id, borrower_id, **kw):
        inside["now"] += 1
        inside["most"] = max(inside["most"], inside["now"])
        await asyncio.sleep(0.02)
        inside["now"] -= 1
    ...
    monkeypatch.setattr(svc, "library_busy_reason", AsyncMock(return_value=None))
    # One stand-in covers both: borrow and return now go through the same
    # dispatch, which is the thing the lock has to serialise.
    monkeypatch.setattr(svc, "_dispatch", slow)
...
async def test_a_returner_waits_for_a_busy_library_too(monkeypatch):
    ...
    monkeypatch.setattr(svc, "_dispatch", AsyncMock(return_value=("dispatched", None)))
