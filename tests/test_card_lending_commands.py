...
async def test_collateral_requires_a_money_server(monkeypatch):
    """Holding tix means the wallet; a guild charging collateral without the
    wallet enabled would fail at transfer time, after the deck was promised."""
    import cogs.card_lending_commands as mod
    monkeypatch.setattr(mod, "is_money_server", lambda gid: False)
    monkeypatch.setattr(mod, "card_library_collateral", lambda gid: 5)
    assert mod.library_gate(_ctx()) is not None


async def test_a_free_library_needs_no_money_server(monkeypatch):
    """Most guilds will lend without collateral and must not be forced to turn
    the wallet on to do it."""
    import cogs.card_lending_commands as mod
    monkeypatch.setattr(mod, "is_money_server", lambda gid: False)
    monkeypatch.setattr(mod, "card_library_collateral", lambda gid: 0)
...
async def test_a_short_wallet_is_told_the_deposit_and_the_gap(monkeypatch):
    ...
    monkeypatch.setattr(mod, "deposit_shortfall",
                        AsyncMock(return_value={"need": 5, "have": 2, "short": 3}))
