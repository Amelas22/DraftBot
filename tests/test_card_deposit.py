...
def rig(monkeypatch):
    ...
    # Which library this server draws on. A deposit books custody to a LIBRARY,
    # not to the room the trade was started in, so one has to exist.
...
async def test_a_forgotten_trade_is_failed_only_when_nothing_crossed(test_db, rig):
    """The serve restarted and lost its job list, so this job can never report.

    Failing it is right ONLY if the trade never happened -- the serve forgets
    the job, not the cards, and for a deposit "failed" is the ledger asserting
    the depositor never handed their cube over. So the boundary is asked
    first, and here it is holding nothing: nothing crossed, and the row can be
    written off. See test_vanished_job_is_not_written_off.py for the other
    half, where it is holding something.
    """
    await svc.start_deposit(GUILD, OWNER, CARDS)
...
async def test_withdrawing_names_the_cards_it_is_asking_for(test_db, rig, monkeypatch):
    """A whole-position withdraw used to name nothing and let the serve settle
    from its own record. That record spans the whole MTGO account, and every
    library's cards sit in it together -- so an unnamed ask is an ask for
    everything, and one depositor's withdrawal could carry away another
    library's stock. Every order names exactly what it is owed."""
    ...
    assert asked is not None, "an unnamed withdrawal spans every library"
    assert {c["name"] for c in asked} == {c["name"] for c in CARDS}
...
async def test_a_deposit_and_a_loan_do_not_cancel_each_other(test_db, rig):
    ...
    assert await svc.held_for(OWNER, LIB) == [{"name": "Auramancer", "qty": 2}], \
...
async def test_adoption_refuses_an_earlier_chunk_s_TRADE_STILL_OPEN(test_db, rig):
    """The same collision as above, caught one step earlier in the trade's life.

    A finished trade is refused because its row is no longer pending. A trade
    that is still OPEN has a pending row -- which adoption is built to accept,
    because re-adopting a trade whose answer we lost is the point of it. So
    chunk two takes chunk one's live trade, reports it as its own, and chunk
    two's real trade (if it opened) is left with no row at all.
    """
    rig.jobs["job-1"] = {"state": "queued"}          # opened, nobody has settled it
    first, _ = await svc.start_deposit(GUILD, OWNER, CARDS)
    ...
    assert (status, job_id) == ("dispatch_unknown", None), \
        "chunk one's live trade is not chunk two's to adopt"
...
async def test_cards_deposited_in_one_server_are_held_in_every_server(test_db, rig):
    ...
    assert await svc.held_for(OWNER, LIB) == [{"name": "Adarkar Valkyrie", "qty": 1},
    ...
    assert scopes == {wallet_service.library_scope(LIB)}, scopes
...
async def test_a_withdrawal_from_another_server_settles_the_same_position(
    ...
    assert await svc.held_for(OWNER, LIB) == [], "the position is closed, not doubled"
