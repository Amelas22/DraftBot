...
_MESSAGES = {
    ...
    "short_cards": "📦 This library can no longer cover that offer. Run `/borrow` again to see what's available.",
    ...
    "wrong_server": "📍 Your deck is waiting in the server you drafted it in — run "
                    "`/borrow` there. (`/return` works from anywhere.)",
...
def describe_deposit_shortfall(figures: "dict[str, int]") -> str:
    ...
    /borrow used to be when the library was short of cards.
...
def library_gate(ctx: Any) -> Optional[str]:
    ...
    guards /mydeck and /return too, which have no cube until a loan is looked
...
async def defer_if_usable(ctx: Any) -> bool:
    ...
    /mydeposits is the exception and is right to be: it only reads a ledger,
        ...
        if outcome == "borrowed":
            ...
            if expected == "returned":
                ...
                await ctx.followup.send(
                    ...
                    f"Run `/borrow` again when you're ready — it costs no more.",
                    ephemeral=True)
